...
5. Final gate (mechanical): run `python "{project-root}/.claude/skills/diy-tools/scripts/diyc.py" check --type openapi --final --json` — exit 0 is the only pass; fix every reported violation and re-run; the JSON receipt (counts included) is the close-out evidence. In plain terms the bar is: valid OpenAPI 3.1 structure, no unconfirmed assumptions, every `x-fr` ID resolving in prd.yaml, every operation user-reviewed.
