...
4. Update mode: apply the change signal to the named operations, bump `updated`, keep `operationId`s stable, re-run diy-viewer (same activation command — append `--instance <name>` when one was resolved) to refresh the HTML.
...
6. Set `x-project.status: final`, re-run diy-viewer (same activation command — append `--instance <name>` when one was resolved), close with one line: path, endpoint count, open assumptions (should be 0).
