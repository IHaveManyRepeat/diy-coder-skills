...
**Write (output):** `{output_dir}/test-review.yaml` 定稿（`status: 已定稿`）；渲染；交付摘要。
...
2. `open_questions`：推不出的判定（惯例语料不足、上下文歧义、要不要给 bonus 的边界情形）落这里，别用 `[假设]` 混过去。
3. 记录改 `status: 已定稿`；`project.updated` 改今天；全文档搜一遍 `[假设]`——本产物定稿禁止该字面量。
...
`exit 0` 是唯一放行。它机械核对：schema / 枚举 / 账本自洽（findings 计数 → deductions → 行相加 → score/grade/recommendation → 维度分）/ `row` ∈ 有效 32 行 / `coverage_gaps` 形态与配型 / `walkthrough` 自洽 / `excluded` 理由三值 / `scope` 非空 / 零 `[假设]` / **convention 行的 `class` 与 `convention_baseline` 对表** / **`recommendations` ≤ 10 条** / **`scope.paths` 不含空测试文件**。违规就地修，重跑，不得跳过。
...
```
测试评审完成：RV-001
- scope：N 个文件（excluded M 个：格式不支持 x / 自动生成 y / 超出范围 z）
...
覆盖缺口（不进评分）：
- 无实现 AC-1.1 → 建议 diy-dev；无测试 AC-1.2 → 建议 diy-test-design；从未运行 TC-1.2.1 → 建议 diy-test-author
- walkthrough: 全覆盖|部分覆盖|已跳过（非全覆盖说明缺源与跳过类）

路由：recommendation=打回 / 要求修改 → 先修测试代码再复审（修复归 diy-dev / diy-test-author）；有保留批准 → 意见交用户定夺；缺口路由是建议，执行归对应技能。
```
