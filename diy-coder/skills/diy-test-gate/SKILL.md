...
全局步骤纪律：一次只加载一个 `steps/` 文件——绝不预载或批量加载六份；整块给出该步输出，不在步骤中间提问；产物散文用 `document_output_language` 写，对话用 `communication_language`。记录在 step 1 建为 `draft`，各步填充同一条记录，step 6 定稿；ID 由你铸造（`TG-###` 三位零填充、稳定不重用不重编号）。
...
4. `steps/04-nfr.md` — NFR 四域证据审计：阈值**禁猜**（唯一合法来源 = 用户会话，其余 UNKNOWN + gap）；ADR 清单（技能内 `adr-checklist.yaml`，29 行）走查；第五个走查维度 = 合规五标准（SOC2 / GDPR / HIPAA / PCI-DSS / ISO27001，聚合 FAIL > PARTIAL > PASS）；跨域风险合成（`reliability×maintainability` / `security×reliability`）。
...
终门（机械判定）：先写 `status: final`，再跑 `python "{project-root}/.claude/skills/diy-test-gate/scripts/gate.py" check --final --project-root "{project-root}" --output-dir "{output_dir}" --json`——exit 0 是唯一放行；修掉每一条报告的违规并重跑；JSON 回执（含计数）就是收尾证据。
...
gates:
  - {id: TG-001, date: YYYY-MM-DD, status: draft|final, scope: story, story: S-x}   # TG-### 稳定不重用；scope 唯一合法值（epic / release 为 v2 推迟项）
    oracle: {source: stories|synthetic, confidence: high|medium|low, items: N, inferred: [<string>], unresolved: [<string>]}   # inferred = synthetic 推断条目清单；source=synthetic 时必填
    coverage:
      ...
      by_level: {unit: N, integration: N, e2e: N}  # 已验证 TC 按 type 计数
      heuristics: [<string>]  # 盲区命中（5 类）
    nfr:
      domains:
        - {name: security|performance|reliability|maintainability, status: PASS|CONCERNS|FAIL|N/A, thresholds: [{name, target, measured, source}], findings: [<string>]}   # 四域固定；source 唯一合法 = 「用户会话 <date>」；findings 兼载两类走查行：合规 `<标准>[@<域>]: PASS|PARTIAL|FAIL|N/A — <依据>`（五标准齐全，聚合 FAIL > PARTIAL > PASS，`check --final` 判 COMPLIANCE_UNRECORDED / COMPLIANCE_AGGREGATE_MISMATCH）与跨域 `<域>×<域>: <判定>`（命中未写判 CROSS_DOMAIN_UNRECORDED）
    ...
    gate:
      ...
      hard_criteria:                   # 一票否决：任一 fail → 整门 FAIL；mutation_score 过渡期记 n/a
        - {name: p0_coverage|overall_coverage|p1_coverage, target: '100%', actual: '<x>%'|n/a, result: pass|fail|n/a}   # 任务书 §4 三行分列：n/a 仅适用 p0_coverage 空档（p1_coverage 空档取 100%）
        - {name: mutation_score, target: '>=90%', actual: '<x>%'|n/a, result: pass|fail|n/a}   # 多 run 聚合 = 全部 run 的 score 最小值；报告缺席时 actual 记 n/a
        - {name: nfr_critical|p0_uncovered, target: 0, actual: N, result: pass|fail}       # nfr_critical = FAIL 域数 − 已豁免域数
      soft_criteria:                   # 任一 fail → 降为 CONCERNS（不影响 FAIL 判定）；estimated 项须附 algorithm
        - {name: business_rule_coverage|boundary_coverage|p0_depth_full|id_chain_resolvable, target: '100%', actual: '<x>%', result: pass|fail}
        - {name: negative_scenario_coverage, target: '>=90%', actual: '<x>%', result: pass|fail}
        - {name: effective_case_ratio, target: '>=95%', actual: '<x>%', result: pass|fail}
      blockers: [{ref: <ID|string>, kind: coverage|nfr|heuristic, why: <string>}]
      waivers: [{ref, approved_by, date, reason, expires, monitoring, fix_owner, fix_target}]   # 8 键固定；仅人工写入；security 域 FAIL 不可豁免
...
3. **门决策自洽：** 任一 `hard_criteria` fail → 必须 `FAIL`；有 `soft_criteria` fail 或任一 NFR 域 `CONCERNS` 或 overlay 命中（`oracle.source: synthetic` 且 `confidence ≠ high`）→ 至少 `CONCERNS`；全 pass 且无域 CONCERNS 且无 overlay → 才可 `PASS`。`check` 会重算并与 `decision` 对账，自造档位必被拒。
4. **阈值不得猜测：** 未知阈值标 `UNKNOWN`（→ 该域 `CONCERNS`，该域不得 PASS）；`thresholds[].source` 只认「用户会话 `<date>`」，其余来源 `check` 拒收。证据不足写 `nfr.gaps`，不写猜测值。
5. **豁免只由人开：** `waivers` 8 键齐备且由用户裁定后才写入；未获裁定的缺口一律计 blocker。`security` 域 FAIL 不可豁免。
