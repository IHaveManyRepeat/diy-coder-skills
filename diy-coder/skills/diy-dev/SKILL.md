...
5. Write `evidence` entries, backfill `test-plan.yaml` TC `status: pass` (with `project.updated` bumped) for every green TC, set task `status: review`, render via diy-viewer, and hand off to diy-review (report path; user confirms review timing).
