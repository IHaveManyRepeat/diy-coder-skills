...
- **Backfill the source of truth — 真源回填 (BUG-012).** A green line in sprint.yaml is only half the write-back: at that same moment, set the TC's `status: pass` in `test-plan.yaml` and bump its `project.updated`. TC status lives in test-plan.yaml; a green run that leaves it stale makes the source of truth lie. **Why:** the 2026-09-13 falsification round found green evidence sitting next to `pending` TCs — human discipline covered the gap, the mechanism did not.
...
5. Write `evidence` entries, backfill `test-plan.yaml` TC `status: pass` (with `project.updated` bumped) for every green TC, set task `status: review`, render via diy-viewer (same activation command — append `--instance <name>` when one was resolved), and hand off to diy-review (report path; user confirms review timing).
