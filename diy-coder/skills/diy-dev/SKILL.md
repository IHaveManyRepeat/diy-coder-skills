...
3. Resolve target: explicit story ID from the user, else the first `待办` task. Tasks not in `待办` are refused with their state named (进行中 → continue via this skill's loop; 待审查 → diy-review; 已完成/已阻塞 → see TDD gate).
4. TDD gate (AC-7.2): target task must have non-empty `test_refs` resolving in test-plan.yaml. A 已阻塞/empty-refs task refuses coding with the message「测试用例缺失，先运行 diy-test-design」and produces zero implementation. Refusal is a stop, not a workaround.
...
- **Activate skipped scaffolds first — 待激活 (B3).** When the scoped TCs' test files carry `skip` markers (diy-test-author red-phase scaffolds), remove them before the first red run: a skipped test yields "skipped", never a failing run, so it produces no red evidence. `skip` there means "not implemented yet" — activation is this skill's first move, not the author's.
...
- **Backfill the source of truth — 真源回填 (BUG-012).** A green line in sprint.yaml is only half the write-back: at that same moment, set the TC's `status: 通过` in `test-plan.yaml` and bump its `project.updated` — the `green` command performs both halves as one atomic batch. TC status lives in test-plan.yaml; a green run that leaves it stale makes the source of truth lie. **Why:** the 2026-09-13 falsification round found green evidence sitting next to `待办` TCs — human discipline covered the gap, the mechanism did not.
- **State ownership is narrow.** This skill writes `待办 → 进行中` at start and `进行中 → 待审查` when green (via `transition`). The last state this skill writes is `待审查`; `已完成` belongs to diy-review. It never touches other tasks.
- **Failed loops stay honest.** If red cannot turn green, stop, set `已阻塞` with `blocked_reason` (via `transition --to 已阻塞 --reason`), and report — do not weaken the test to pass.
...
- Any judgment call (scope exemption, partial coverage) carries the `[假设]` prefix in the YAML value.
...
2. Take the task: `python "{project-root}/.claude/skills/diy-tools/scripts/diyc.py" transition --story <S-x> --to 进行中 --json` (writes the status and bumps `project.updated` in one atomic write).
