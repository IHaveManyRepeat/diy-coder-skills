...
3. Mode. File absent → **全量**. Present → read its `scan` block and offer **重扫** (re-scan everything, keep the recorded date as the cutoff), **深挖** (exhaustive on one area), or **Cancel** (keep the file as-is).
...
5. `steps/05-deep-dive.md` — exhaustive deep dive of one area (mode `深挖`): literal full-file review, sampling forbidden.
...
4. `深挖` mode requires literal full-file review. Sampling, guessing, or relying on tooling output alone is FORBIDDEN: every file in scope is read.
