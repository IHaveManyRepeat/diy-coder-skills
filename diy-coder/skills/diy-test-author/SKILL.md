...
2. 硬门：`{output_dir}/test-plan.yaml` 的 `project.status` 必须是 `final`。不满足 → 一行拒绝（点名缺什么）+ 零产出 + 路由 diy-test-design。
...
1. `steps/01-preflight.md` — 探测与门禁：`author.py detect` 认栈与框架、test-plan final、范围内 TC 在场；**无框架 → 一行拒绝 + 路由 diy-test-framework**（本技能不装框架）。
2. `steps/02-scope.md` — 范围选择（story / type / priority / 显式 TC 列表），只取 `status: pending` 的 TC。
3. `steps/03-generate.md` — 按 TC 的 `type` 分派生成（unit / integration / e2e），连同夹具与工厂；每用例上方一行 TC 锚。
...
2. 交付门：全部 `skip` 且未执行。附加门：范围内每个 TC 的 `technique` / `kill_target` 非空，且 `status` 必须是 `pending`（`fail` 是已激活测试、`pass` 已收口，都不得被脚手架覆盖）。范围内无可做 TC（全 `pass` / 空）→ 一行报告「范围内无待处理 TC」+ 零产出，**不静默空跑**。
