...
# Azure DevOps 测试流水线（diy-test-framework 渲染；要改请改技能内模板，不要改本文件）
# 质量门（事实源 = test-framework.yaml 的 ci.gates）：P0 覆盖 {{P0_GATE}} / P1 覆盖 {{P1_GATE}}
# 分片：matrix 注入 SHARD_INDEX；TEST_CMD 取值可带分片参数（如 `npx playwright test --shard=$(SHARD_INDEX)/4`）
# 缓存：Cache@2 依赖 + 浏览器各一份，键含锁文件哈希，restoreKeys 为回退前缀
        ...
        steps:
          ...
          - task: Cache@2
            ...
            inputs:
              ...
              restoreKeys: {{CACHE_RESTORE_KEYS}}
          - task: Cache@2
            displayName: "Cache browsers"
            inputs:
              key: {{BROWSER_CACHE_KEY}}
              path: {{BROWSER_CACHE_PATH}}
              restoreKeys: {{BROWSER_CACHE_RESTORE_KEYS}}
          - script: {{INSTALL_CMD}}
            displayName: "Install dependencies"
          - script: {{BROWSER_INSTALL}}
          ...
          - publish: test-results/
            ...
            condition: failed()
          - publish: {{REPORT_PATH}}
            artifact: test-report-$(SHARD_INDEX)
            condition: failed()
...
#     env: { TEST_GREP: $(TEST_GREP_PARAM) }
# ❌ NEVER
#   - script: npx playwright test --grep "${{ parameters.testGrep }}"
