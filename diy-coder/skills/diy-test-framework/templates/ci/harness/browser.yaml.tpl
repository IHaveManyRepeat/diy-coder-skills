...
# Harness CI 测试流水线（diy-test-framework 渲染；要改请改技能内模板，不要改本文件）
# 质量门（事实源 = test-framework.yaml 的 ci.gates）：P0 覆盖 {{P0_GATE}} / P1 覆盖 {{P1_GATE}}
# 分片：repeat.strategy 并行四份，SHARD 由 <+strategy.iteration> 给出；TEST_CMD 取值可带分片参数
# 缓存：Cache Intelligence（stage.spec.caching）——依赖与浏览器同一份缓存键，路径须落在 /harness 下
          ...
          execution:
            steps:
              ...
              - step:
                  ...
                  spec:
                    shell: Sh
                    command: echo "test-results/ 与 {{REPORT_PATH}} 上传为失败产物（retention 30 天）"
          rollbackSteps: []
          repeat:
            strategy: Parallel
            items:
...
# ✅ SAFE —— 输入作为参数进入固定命令
#   command: npx playwright test --grep "$TEST_GREP"
#   envVariables: { TEST_GREP: <+input>.testGrep }
# ❌ NEVER
#   command: npx playwright test --grep "<+input>.testGrep"
