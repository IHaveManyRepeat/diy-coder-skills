...
# GitHub Actions 测试流水线（后端面：无浏览器安装步骤；diy-test-framework 渲染）
# 质量门（事实源 = test-framework.yaml 的 ci.gates）：P0 覆盖 {{P0_GATE}} / P1 覆盖 {{P1_GATE}}
# 分片：matrix.shard 已注入 SHARD 环境变量；TEST_CMD 取值可带分片参数（按 runner 惯例）
# 缓存：依赖一份，键含锁文件哈希；restore-keys 为回退前缀（失配可复用旧缓存）
...
#   - run: python -m pytest -k "${{ inputs.test-filter }}"
