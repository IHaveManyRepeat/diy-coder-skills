# placeholders: RUNTIME_SETUP_CMD INSTALL_CMD LINT_CMD TEST_CMD P0_GATE P1_GATE CACHE_LOCKFILE CACHE_PATH NOTIFY_SECRET
# GitLab CI 测试流水线（后端面：无浏览器安装步骤；diy-test-framework 渲染）
# 质量门（事实源 = test-framework.yaml 的 ci.gates）：P0 覆盖 {{P0_GATE}} / P1 覆盖 {{P1_GATE}}
# 分片：test 作业 parallel: 4，分片序号用内置变量 $CI_NODE_INDEX
# 缓存：cache:key.files 由锁文件内容派生（锁文件变即换键；未命中则按 key 前缀回退到最近一次缓存）
...
# ✅ SAFE —— 输入作为参数进入固定命令
#   script:
#     - python -m pytest -k "$TEST_FILTER"
# ❌ NEVER
#   script:
#     - python -m pytest -k "$CI_TRIGGER_FILTER"
