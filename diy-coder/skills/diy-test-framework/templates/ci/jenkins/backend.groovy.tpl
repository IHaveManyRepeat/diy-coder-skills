...
// Jenkinsfile 测试流水线（后端面：无浏览器安装步骤；diy-test-framework 渲染）
// 质量门（事实源 = test-framework.yaml 的 ci.gates）：P0 覆盖 {{P0_GATE}} / P1 覆盖 {{P1_GATE}}
// 分片：failFast false 的四路并行分支，各自注入 SHARD
// 缓存：Jenkins 无声明式缓存原语——依赖缓存由 agent 工作区保留 + 工具自动安装覆盖；
//       约定的缓存键 = {{CACHE_KEY}}（锁文件哈希口径，与其他平台一致），依赖缓存路径 = {{CACHE_PATH}}。
//       换 agent 或清理工作区后首轮为冷缓存。
...
// ✅ SAFE —— 参数作为参数进入固定命令
//   sh 'python -m pytest -k "$TEST_FILTER"'
// ❌ NEVER
