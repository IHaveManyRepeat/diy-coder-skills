...
# ↑ 中文：搭测试框架与 CI 质量流水线——探测技术栈，与用户共定框架与 CI 平台，从技能内 templates/ 逐字节渲染脚手架与流水线文件，选型与生成台账落 test-framework.yaml，并机械校验 CI 文件对 test-plan 的 static_checks。用户说 "lets setup test framework" / "lets setup CI pipeline" / "I want to create quality gates" 时触发。
phase: 3-solutioning
precededBy: []
...
**边界。** 本技能建基建：框架配置 / 脚手架 / CI 流水线 / 冒烟自检。测试代码归 `diy-test-author`、测试计划归 `diy-test-design`；部署文件（Dockerfile / 部署脚本）不在 scope。**项目文件只从技能内 `templates/` 渲染**——禁 LLM 手写冒充（手写产物在重入与 check 面上不可机械校验）。
...
2. `steps/02-select.md` — 框架/平台/单元层选型与理由 + 用户确认；产出 substitutions 取值表（取值 = detect 结果 + `static_checks` 的 阻断 命令 + 门禁阈值）。
3. `steps/03-scaffold.md` — 写 框架 面 plan → `scaffold` 调用一次 → 既有配置文件走 LLM 手改通道（台账 `action: 更新`）→ 执行回执 `pending_commands`。
4. `steps/04-pipeline.md` — 写 CI 面 plan（注入防护段随模板逐字保留）→ `scaffold` 调用一次 → 执行回执命令。
5. `steps/05-verify.md` — 冒烟自检（静态检查 / 示例测试 / CI 语法；产物面失败修后重跑，修不了即 HALT）+ `chmod +x` + hook 合并经 `defer-add` 入队。
...
setups:
  - id: TF-001                # TF-### 稳定：顺序递增，永不重编号、永不复用
    ...
    status: 草稿|已定稿
    mode: 框架|CI|两者
    stack: {type: 前端|后端|全栈|移动端, language: <string>, package_manager: <string>}
    framework: {name: <string>, runner: <string>, reason: <string>}   # 选型理由（源 02 步保留）
    unit_layer: {name: <string>}   # 移动端 / 全栈 时必填
    files:                        # 生成/修改的项目文件清单（对齐 path: 口径）
      - {path: <relative>, kind: 脚手架|配置|CI|钩子|脚本|文档, action: 新建|更新}   # 新建 = 引擎新写；更新 = 既有文件由 LLM 手改
    checks:                       # 已执行的安装/自检命令与结果（result: 失败 时 note 必填）
      - {command: <string>, result: 通过|失败, note: <string>}
    ci:
      platform: github-actions|gitlab-ci|jenkins|azure-devops|harness|none
      file: <relative>            # platform=none 时省略
      stages: [<string>]          # 静态检查|测试|契约|预热|报告
      ...
      static_check_alignment:     # 引用式：以 order 引用 test-plan.yaml 的 static_checks（禁抄录命令内容）
        - {order: N, in_ci: true|false}   # 阻断 层必录、记录不阻断 层可选
...
3. **冲突即停**：目标已存在且内容不同 → 引擎回滚本次已写文件 + 整条拒绝（`FILE_CONFLICT`）；既有文件的修改只走 LLM 手改通道并记 `action: 更新`。
4. **终门（机械判定）**：先置 `status: 已定稿`（gate 检的就是它），再运行 `python "{project-root}/.claude/skills/diy-test-framework/scripts/test_framework.py" check --final --project-root "{project-root}" --output-dir "{output_dir}" --json`——exit 0 是唯一放行；修掉每一条报告的违规并重跑；JSON 回执（含计数）就是收尾证据。渲染与收尾等 exit 0。
5. **CI 三方对齐**：CI 路径一律相对 + 正斜杠（与 correct-course `path:` 同口径）；阻断 层 `static_checks` 命令须以独立命令形态出现在 CI 文件，且台账 `in_ci` 与 `check` 现场重算一致（**重扫不采信台账**）；`ci.gates` 阈值经 plan 注入 CI 文件文本。
