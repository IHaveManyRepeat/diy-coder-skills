...
**Write (output):** 会话内的选型基线与前置结论；本步不写任何文件（记录 `draft` 在 step 3 之前不落盘）。
...
- `stack.type` ∈ `frontend|backend|fullstack|mobile`（mobile 优先判定）；`stack.languages` / `stack.package_manager` / `stack.manifests` 是事实，**禁手推**。
- `existing.framework` / `existing.ci` = 项目既有基建；`conflicts` = 需要用户裁决的替换类冲突。
- `suggested` = 选型建议（框架 / 平台 / profile）；**建议不是结论**——step 2 由用户确认。
- `templates.framework_supported` / `templates.ci_supported` 为 false（当前 = mobile 面）→ **HALT**：一行报告「哪个栈、为什么无模板覆盖」+ 在回报/摘要登记，本技能零产出（禁 LLM 手写脚手架）。
...
- 用户要建框架/脚手架 → `framework`；要建 CI/流水线/质量门 → `ci`；两者都要 → `both`。
- 说不清 → 停下问一次（给这三个选项），不猜。
...
- **冲突门**：`conflicts` 非空 → 停下把冲突摆给用户（源「已有冲突框架」HALT 保留，替换 = 内容决策）：
  - 用户确认替换 → 由用户在项目侧移除或改名旧文件后**重入**（旧路径此后记 `action: new`）；
  - 用户不改 → 冲突文件不写、不删、不覆盖，其余文件照常推进（摘要记明）。
- **ci 模式附加门**（仅 `mode=ci`；`mode=both` 是同一会话内先 framework 段后 CI 段，不设前置门）：
  ...
  - **就绪凭据**：**满足任一条即过**——① 台账任一 `setups[]` 含 `kind ∈ {scaffold, config}` 的 `files[]` 条目且这些文件当前在场；② `existing.framework` 非空。两条皆不满足 → HALT + 一行报告 + 路由本技能 `framework` 模式（源「先跑 framework」保留）。
