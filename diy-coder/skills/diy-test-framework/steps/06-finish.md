...
## 1. 落盘定稿
本记录置 `status: final`（gate 检的就是它）；新增记录追加进 `setups[]`，改既有记录时补一条 `revisions`（`{date, change, reason}`）。未裁定的未决项落 `open_questions`，**零 `[ASSUMPTION]`**。
...
摘要（收尾证据，写给用户）：
- 选型：框架 / 平台 / profile / 理由；
- 生成文件清单（`files[]`，`new` / `update` 分列）与 `skipped`（幂等重入）说明；
- 执行过的命令（`checks[]`，fail 项带 note）+ 人工处置项；
- CI 对齐结论：blocking 条目覆盖、阈值注入、`platform: none` 或 `test-plan` 缺席导致的跳过（含 warning 原文）；
