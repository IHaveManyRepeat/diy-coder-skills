...
**Read (input):** the record at `status: ready`; its `code_map` and `tasks`; the code those paths name.
**Write (output):** the code; the record's `baseline`, `status: in-progress`, `tasks[].done`, `verification.commands[].result`.

## Precondition

The record exists on disk with `status: ready` (a resume may find `in-progress` — continue from the unfinished tasks). Otherwise HALT and ask which record to work on; never guess one.
...
- Respect `boundaries`: `always` are invariants, `never` is forbidden, and anything under `ask_first` HALTs the run for a human decision before it happens.
- Keep the diff inside `code_map`; a file that turns out to be needed but is not listed is an amendment to the record (and, if the sealed sections are wrong, a step-4 `bad_spec` loopback — never a silent extra edit).
