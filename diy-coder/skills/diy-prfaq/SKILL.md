...
5. `steps/05-verdict.md` — Stage 5: the verdict (已锤炼 / 欠火候 / 地基裂缝), the polish, the distillate, the final gate and the terminal close. Revision loops back to the relevant stage.
...
prfaq:
  ...
  concept_type: 商业|内部|开源|社区   # detected in Stage 1; calibrates Stage 3-4 question framing
  ...
  verdict: {strength: 已锤炼|欠火候|地基裂缝, narrative}   # narrative, not a score
...
3. Inferred or low-confidence answers (headless especially) carry the `[假设]` prefix in the YAML value — quote the value when the marker leads it (`a: '[假设] ...'`; an unquoted leading `[` breaks YAML). The final gate requires zero: clear the prefix or turn the item into an explicit `distillate.open_questions` entry.
