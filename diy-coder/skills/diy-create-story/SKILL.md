...
2. Settle the target story: an explicit story ID from the user, else the first non-`已完成` task in `{output_dir}/sprint.yaml` `tasks[]`, else ask. Then run the deterministic opener:
...
3. `steps/03-code-survey.md` — survey every file the story touches: read each `更新` target in full and record `current_state` + `preserve`; place `新建` files by the existing project structure.
  ...
  - id: SC-001                  # SC-### — sequential, stable, never renumbered or reused
    ...
    files:                      # files this story will touch (surveyed, never guessed)
      - path: <relative>        # CWD-relative, forward slashes
        action: 新建|更新
        why: <one-line>
        current_state: <string> # 更新 only, required: what the file does today
        preserve: <string>      # 更新 only, required at --final: behavior not to break
...
4. Gate (zero output): `stories.yaml` missing, unparsable or `status != 已定稿`, or the target story absent → refuse, relay the reason plus the route (diy-epics-stories) or the `suggestions` list, and write nothing. `sprint.yaml` / `test-plan.yaml` / `architecture.yaml` absent → warnings only: the run continues on a thinner reference surface.
5. The file survey is not optional: every `更新` entry carries `current_state`, and at final `preserve`. "READ FILES BEING MODIFIED" is the source workflow's non-negotiable — skipping it is the primary cause of implementation failures and review cycles.
6. `[假设]` prefixes an inferred claim; final requires zero — clear it or land it as an open question closed with the human. `open_questions` must be empty at final, or each entry prefixed `[CLOSED]` with the call taken.
