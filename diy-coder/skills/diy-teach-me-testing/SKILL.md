...
<!-- description 中文对照：按 7 节结构化课程渐进教测试，带跨会话进度跟踪。把 diy 测试链当课程来跑：入口画像采集 → 按角色适配的讲解 → 每节 3 题测验（通过线 score >= 70）→ 课堂笔记 → 7 节修满后出结业摘要。一个 output_dir 一份学员进度，落 learning-progress.yaml、跨会话续学。不生成测试代码、不移动任务状态。用于用户说 "lets learn testing" 或 "I want to study test practices"。 -->
...
2. 分流（唯一入口表，与 `status` 回执的 `entry_step` 同源）——先跑 `status`，按回执选一个 step：
   ...
   中断恢复一律先进 `03-hub.md`（Hub-and-spoke：任何一节的入口都在 Hub 选择），in-progress 节在仪表盘标示，**不直接跳节**。进度文件已存在 → 强制走 resume：不得跳过 `status` 检查直接开会话，也不得覆盖重建。
...
learner:
  role: QA|Dev|Lead|VP|null          # init 后可空，02-assess 补全（写通道 = update --learner）
  experience: <string|null>          # beginner|intermediate|experienced；assessed 非空后必填
  goals: [<string>]
  pain_points: [<string>]
...
sessions:
  - id: 1                            # 整数 1-7，与 curriculum.yaml 对齐（session 编号即键）
    ...
    score: <N|null>                  # 0-100 整数 = round(答对数*100/3)；仅 completed 非空；session 7 恒 null（无 quiz）
    topics_explored: <N|null>        # 仅 session 7 非空（写通道 = update --session 7 --topics）；完成判据 >= curriculum min_topics
    notes: <relative path|null>      # 固定命名 notes/session-<NN>.md（相对 output_dir、正斜杠）；completed 节必填且文件在场
sessions_completed: N                # 派生（引擎写）= status == completed 计数
completion_percentage: N             # 派生（引擎写）= floor(sessions_completed*100/7 + 0.5)
next_recommended: <1-7|null>         # 派生（引擎写）= 最小 id 未 completed 节；全 completed → null
...
5. 终门（机械判定）：`python "{project-root}/.claude/skills/diy-teach-me-testing/scripts/progress.py" check --final --project-root "{project-root}" --output-dir "{output_dir}" --json`——exit 0 是唯一放行（7 节全 completed 且 `summary.generated` 为真且摘要文件在场）；修掉每条违规并重跑，回执 `counts` 就是收尾证据。
