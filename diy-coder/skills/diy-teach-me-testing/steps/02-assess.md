...
1. **角色**（必填）：`QA`（QA 工程师 / 测试工程师 / SDET）、`Dev`（软件开发者）、`Lead`（技术负责人 / 工程经理）、`VP`（研发负责人 / 总监）。答别的 → 重申四值，重复问到达标。用来选 `curriculum.yaml` 的 `roles.<R>.teaching_adaptations`。
2. **测试经验**（必填）：`beginner`（刚接触）、`intermediate`（写过测试、想提高）、`experienced`（底子扎实、想学进阶）。这三值是 `learning_paths` 的键——它决定推荐顺序。
...
```
python "{project-root}/.claude/skills/diy-teach-me-testing/scripts/progress.py" update --learner --role <QA|Dev|Lead|VP> --experience <beginner|intermediate|experienced> --goals "<目标>" [--pain-points "<痛点>"] ... --project-root "{project-root}" --output-dir "{output_dir}" --json
```
