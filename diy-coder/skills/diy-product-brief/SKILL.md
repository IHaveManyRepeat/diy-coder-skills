...
2. Detect intent — **新建** / **更新** / **校验**. Ask when it is ambiguous and the run is interactive; headless: infer from the ask and from whether `{output_dir}/brief.yaml` exists. Then take the precondition and the route from the engine:
...
**Headless.** Do not ask; complete the intent from what is provided, what exists in `brief.yaml`, or what you can discover. Ambiguity that survives inference halts `blocked` — zero writes, one-line reason, route. End with the JSON status block `{"status": "complete|blocked", "intent": "新建|更新|校验", "artifacts": ["{output_dir}/brief.yaml"], "open_questions": [], "offer_to_update": false}`; omit keys for artifacts not produced, and `offer_to_update: true` is mandatory for 校验. No `external_handoffs` key — diy delivers local files only.
...
1. `steps/01-discovery.md` — 新建: invite the brain dump and existing material, read the stakes, offer fast path vs coaching path, put the workspace on disk as a `status: 草稿` brief.
2. `steps/02-draft.md` — draft section by section; shape follows the product, not the template; decisions and addendum written live; `[假设]` tags while drafting.
...
4. `steps/04-update.md` — 更新: reconcile a change signal, surface conflicts with prior decisions before changing; a fundamental change is offered 新建 instead.
5. `steps/05-validate.md` — 校验: honest critique against the brief's own purpose, cite specific lines, return inline, always offer to roll findings into an update.
...
```yaml
project: {name, status: 草稿|已定稿, created, updated}   # status is the document-level flag the final gate inspects
brief:
  title: string
  stakes: 个人兴趣|内部|投资人|公开   # risk calibration: how hard to push
  ...
  assumptions: [string]                    # [假设]-prefixed while drafting; empty once 已定稿
...
decisions:                                 # canonical memory: every decision, change and override, as it happens
  - {id: BD-001, date: YYYY-MM-DD, decision, rationale, status: 生效|已反转}   # BD-### — sequential, stable, never renumbered or reused
...
2. Gate: 新建 is the only intent that runs without an existing file; 更新/校验 on a missing brief.yaml are refused by the engine with zero writes — relay the reason and its route, never create the file to unblock yourself.
3. Persistence is real-time: the workspace is on disk from the moment 新建 intent is confirmed, and the user knows the path. There is no `.decision-log.md` here — `decisions` in brief.yaml is the canonical memory and audit trail: every decision, change and override (headless overrides included) is recorded as it happens.
4. Coaching posture: read the room on stakes — a `个人兴趣` project does not need `投资人`-grade rigor, a VC pitch does; never fabricate a moat.
