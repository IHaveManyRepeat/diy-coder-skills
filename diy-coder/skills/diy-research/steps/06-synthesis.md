...
**Write (output):** the record's `synthesis` (`executive_summary` / `key_points` / `open_questions`) and `status: final`; the rendered view; the closing summary.
...
- What the findings mean against the recorded `goals` (goal by goal) and which recommendation findings (`technical`/`domain` dimensions carry an `recommendations` area) survive scrutiny.
...
- `open_questions` — research gaps, limitations, and what further investigation would resolve. Source limitations and low-confidence areas belong here rather than being smoothed over; this is where draft-time `[ASSUMPTION]` markers must land before finalizing.
...
- Every finding has at least one source with `url` + `accessed`, and its `confidence` matches the evidence (multiple authoritative sources → `high`; single or partial → `medium`; uncertain or dated → `low`).
...
1. Write `status: final` on the record first — `final` is what the gate inspects, not a product of it.
