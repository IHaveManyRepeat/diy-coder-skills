...
1. Read `{project-root}/diy-coder.yaml`; resolve `communication_language`, `paths.output_dir`. Speak it for the entire run. Instance resolution (FR-4.5/D-9): if the activation args carry an instance name (`--instance <name>` or 「实例 <name>」), resolve `output_dir` as `<output_dir>/<name>/` (the directory IS the instance); this run reads ONLY that instance dir. No instance arg → mainline flat path. Instance name must match `[A-Za-z0-9][A-Za-z0-9._-]*`, else refuse.
2. Run exactly once, without changing the working directory:

```bash
uv run --with pyyaml "{project-root}/.claude/skills/diy-help/scripts/help.py" --project-root "{project-root}"
```
...
1. If `uv` is unavailable, report the error and suggest `python help.py` with PyYAML installed. Do not silently fall back.
